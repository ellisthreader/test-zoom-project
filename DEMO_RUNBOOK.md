...
2. **Workspace Setup**
   Say: "The first step is customer alignment: goal, channels, guardrails, and
   launch targets."

3. **Integrations**
   Say: "The product makes the agent's system boundaries visible: CRM,
   telephony, knowledge retrieval, and helpdesk actions."

4. **Voice Lab**
   Change the provider, move the latency slider, and toggle barge-in.
   Say: "This is where teams tune whether the agent feels usable on live calls."

5. **Evaluation Suite**
   Click the three scenarios:
   - Duplicate billing charge.
   - Angry cancellation caller.
   - Prompt-injection refund attempt.

   Say: "The evaluation suite turns production readiness into a measurable gate."
...
- Walk through CRM, telephony, knowledge, and helpdesk.
- Say these would become production adapters in a backend implementation.
- Emphasize visibility: operators need to know what the agent can read, write,
  route, and audit.

### 2:00-2:45 - Voice Lab

- Change provider.
- Move response-start latency.
- Toggle barge-in.
- Talk about ASR partials, endpointing, TTS start, silence handling, and
  interruption recovery.

### 2:45-4:10 - Evaluation Suite

- Start on duplicate billing charge.
- Show Conversation, Tool trace, and Eval gate tabs.
- Switch to angry cancellation caller and explain supervised rollout.
- Switch to prompt-injection refund attempt and show guardrails.
- Tie the flow back to readiness, containment, and resolution.
...
- Say the report is the operational artifact for launch ownership.
